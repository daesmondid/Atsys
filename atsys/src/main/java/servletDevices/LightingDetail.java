...
/**
 * Servlet implementation class LightingDetail
 */
@WebServlet("/LightingDetail")
public class LightingDetail extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public LightingDetail() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		new dao.LogicBoard().get(new dao.Lighting().get(id).getLogicBoard().getId()).checkConnection();
		...
		request.setAttribute("lighting", new dao.Lighting().get(id));
		request.setAttribute("ticketList", new dao.Ticket().list());
		RequestDispatcher view = request.getRequestDispatcher("/LightingDetail.jsp");
