				...
				<div class="section nomargin">
					...
					<div class="container clearfix">
						...
						<div class="row common-height clearfix">
							...
							<c:forEach items="${doorLockList}" var="doorLock" varStatus="loop">
								<c:choose>
									<c:when test="${doorLock.logicBoard.user.id == requestScope.sessionUser.id}">
										<div class="col-md-4 col-sm-6 bottommargin-sm" data-animate="fadeInRight" data-delay="${500 + (200 * loop.index)}">
											<div class="white-section">
												<label><c:out value="${doorLock.name}"></c:out></label>
												<c:choose>
												  <c:when test="${doorLock.logicBoard.connectionStatus == 'UNKNOWN'}">
												    <span class="label label-default" style="position:absolute; right: 35px; top: 25px;">Unknown</span>
												  </c:when>
												  <c:when test="${doorLock.logicBoard.connectionStatus == 'OFFLINE'}">
												    <span class="label label-default" style="position:absolute; right: 35px; top: 25px;">Offline</span>
												  </c:when>
												  <c:when test="${doorLock.logicBoard.connectionStatus == 'ONLINE'}">
												    <span class="label label-success" style="position:absolute; right: 35px; top: 25px;">Online</span>
												  </c:when>
												</c:choose>
												<center>
													<h2 id="switch-label-${loop.index}">
														<c:choose>
														  <c:when test="${doorLock.lockStatus == 'UNKNOWN'}">
														    Unknown
														  </c:when>
														  <c:when test="${doorLock.lockStatus == 'UNLOCKED'}">
														    <span style="color: red;">Unlocked</span>
														  </c:when>
														  <c:when test="${doorLock.lockStatus == 'LOCKED'}">
														    Locked
														  </c:when>
														</c:choose>
													</h2>
		...
		$(document).ready(function () {
			...
			<c:forEach items="${doorLockList}" var="doorLock" varStatus="loop">
				$("#switch-toggle-${loop.index}").change(function(){
					if (this.checked) {
						$.post("ClientDoorLockEditLockProcess",
							    {
							        id: "${doorLock.id}",
							        lock: 3
							    },
						...
						$("#switch-label-${loop.index}").html("Locked");
				    } else {
				    	$.post("ClientDoorLockEditLockProcess",
							    {
							        id: "${doorLock.id}",
				    	...
				    	$("#switch-label-${loop.index}").html("<span style=\"color: red;\">Unlocked</span>");
				    }
			    });
			</c:forEach>
